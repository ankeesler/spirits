...
func (h speedHeap) Less(i, j int) bool {
	...
	if h[i].ticks == h[j].ticks {
		return h[i].s.Agility > h[j].s.Agility
	}
...
func newSpeedHeap(spirits []*spirit.Spirit) speedHeap {
	var h speedHeap
...
func (h *speedHeap) next() (*spirit.Spirit, *spirit.Spirit) {
...
func (h speedHeap) getOther(s *spirit.Spirit) *spirit.Spirit {
...
func newSpeedState(s *spirit.Spirit) *speedState {
	return &speedState{s: s, ticks: float64(1) / float64(s.Agility)}
}
